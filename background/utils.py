...
STATIC_BG_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
...
def create_watermark_layer(W, H, bg_color_hint=None):
    ...
    # Center horizontally, position in upper area of image (25% from top)
    cx = W // 2
    top_y = max(int(H * 0.12), 20)
...
def _add_contact_shadow(canvas, car_x, car_y, car_w, car_h, BG_W, BG_H,
    ...
    try:
        if bg_is_light:
            side_alpha = 18   # very light — barely visible, just natural depth
        else:
            side_alpha = 12

        # Left side shadow
        layer_left = Image.new("RGBA", (BG_W, BG_H), (0, 0, 0, 0))
        left_w = max(30, int(car_w * 0.06))
        left_h = max(20, int(car_h * 0.55))
        ...
        layer_left = layer_left.filter(ImageFilter.GaussianBlur(radius=max(12, int(car_w * 0.025))))
        ...
        dr.ellipse([right_x - left_w // 2, car_y + car_h // 2 - left_h // 2,
                   ...
                   fill=(0, 0, 0, side_alpha))
        layer_right = layer_right.filter(ImageFilter.GaussianBlur(radius=max(12, int(car_w * 0.025))))
...
def _load_logo_clean():
    ...
    for lp in logo_candidates:
        ...
        try:
            ...
            # Only remove white/near-white background if logo is not already transparent.
            # Check if there are substantial transparent pixels already.
            already_transparent = float(np.sum(a < 10)) / (arr.shape[0] * arr.shape[1])
            if already_transparent < 0.15:
                # Logo has a white/solid background — remove it
...
def add_logo_overlay(img_rgb, car_top_y, canvas_w, canvas_h, opacity=0.42):
    """
    Overlay Caryanams logo centered horizontally, above car roof.
    Logo is ~36% of canvas width, semi-transparent (light, not dark).
    car_top_y: the y-coordinate of the car roof on the canvas.
    """
    logo = _load_logo_clean()
    if logo is None:
        return img_rgb

    logo_w = int(canvas_w * 0.36)
    logo_h = int(logo_w * logo.size[1] / logo.size[0])
    logo   = logo.resize((logo_w, logo_h), Image.LANCZOS)

    # Apply opacity — keep light/subtle so it doesn't overpower the car
    la = np.array(logo, dtype=np.float32)
    la[:,:,3] = la[:,:,3] * opacity
    logo = Image.fromarray(np.clip(la, 0, 255).astype(np.uint8), 'RGBA')

    # Center horizontally; bottom of logo sits ~18px above car roof
    logo_x = (canvas_w - logo_w) // 2
    logo_y = max(8, car_top_y - logo_h - 18)
...
def composite_car_on_static_bg(fg_img, car_size_percent=85, lighting=1.0, tint_color=None,
    ...
    - NO studio/static BG image — always pure white #FFFFFF
    ...
    if os.path.exists(STATIC_BG_PATH):
        studio = Image.open(STATIC_BG_PATH).convert("RGBA").resize((out_W, out_H), Image.LANCZOS)
        canvas = studio
    else:
    ...
    # Flatten onto studio background RGB
    if os.path.exists(STATIC_BG_PATH):
        studio_base = Image.open(STATIC_BG_PATH).convert("RGB").resize((out_W, out_H), Image.LANCZOS)
    else:
        studio_base = Image.new("RGB", (out_W, out_H), (255, 255, 255))
    studio_base.paste(canvas, (0, 0), canvas)

    # ── Mirror reflection: flip bottom 28% of car, paste below car, dark tint ──
    try:
        studio_base = _add_mirror_reflection(studio_base, fg, car_x, car_y,
...
def process_plate_and_background(image_path, output_path, mode='caryanams', manual=None, car_size_pct=75, image_category='exterior', quad=None, quality='ultra'):
    ...
    try:
        ...
        work_path = output_path.replace('.png', '_work.png')
        if image_category == 'bg_only':
            # BG Only: skip plate hiding, use original image directly
            import shutil
